...
function anipat_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'anipat' ),
        'next_string'     => esc_html__( '&raquo;', 'anipat' ),
        'before_output'   => '<div class="anipat-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'anipat_pagination_defaults', $defaults )
    );
