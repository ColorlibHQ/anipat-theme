...
function anipat_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'anipat' ) . '</span></li>';
...
// Blog categories
function anipat_featured_post_cat(){
...
// social media
if ( ! function_exists( 'anipat_social' ) ) {
	function anipat_social( $args = array() ) {
		...
		$url = anipat_opt( 'anipat_social_url' );
...
// header cart count
function anipat_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( ANIPAT_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'anipat' ); ?>">
...
function anipat_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'anipat_contact7_form_content', 10, 2 );