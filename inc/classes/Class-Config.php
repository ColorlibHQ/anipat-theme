...
final class Anipat {
	...
	private function setup() {
		...
		$enqueu          = new anipat_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->anipat_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'anipat_content_width', 751 );


		// text domain for translation.
		load_theme_textdomain( 'anipat', ANIPAT_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'anipat_site_logo_146x36', 146, 36, true );
		
		// Hero pet thumbnail size
		add_image_size( 'anipat_hero_pet_thumb_947x745', 947, 745, true );
		
		// About thumbnail size
		add_image_size( 'anipat_about_thumb_514x669', 514, 669, true );
		
		// Testimonial thumbnail size
		add_image_size( 'anipat_testimonial_thumb_131x149', 131, 149, true );
		
		// Team thumbnail size
		add_image_size( 'anipat_team_thumb_360x317', 360, 317, true );
				
		// Latest post thumbnail size
		add_image_size( 'anipat_widget_post_thumb', 80, 80, true );

		// Single blog post image size
		add_image_size( 'anipat_single_blog_750x375', 750, 375, true );
		add_image_size( 'anipat_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu' => esc_html__( 'Primary Menu', 'anipat' ),
			'our-services' => esc_html__( 'Our Services', 'anipat' ),
			'quick-links' => esc_html__( 'Quick Links', 'anipat' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = ANIPAT_DIR_CSS_URI;
		$jsPath  = ANIPAT_DIR_JS_URI;
		...
		$scripts = array(
			'style'   => array(
				...
				array(
					...
					'dependency' => array(),
					'version'    => $this->anipat_version,
				),
				array(
					'handler' => 'anipat-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'anipat' ) ) {
			
			$font_families = array(
				'Open+Sans:300i,400,400i,600,700',
				'Poppins:200,200i,300,300i,400,500,500i,600,700,800,900',
			);
	...
	private function customizer_init(){
		...
		// Instantiate anipat theme customizer
		$anipat_theme_customizer = new anipat_theme_customizer();
	}
	...
	public function set_elementor_flag(){
		update_option( 'anipat_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'anipat-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'anipat_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'anipat-elementor-default-style',  ANIPAT_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->anipat_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'anipat_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'anipat-elementor-notice', ANIPAT_DIR_JS_URI.'anipat-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'anipat-elementor-notice',
				'anipatElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'anipat-elementor-notice-nonce' ),
				)
			);
		}
